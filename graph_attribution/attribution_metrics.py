...
def nodewise_metric(
        f) -> Callable[[List[GraphsTuple], List[GraphsTuple]], np.ndarray]:
...
def _validate_attribution_inputs(y_true: List[GraphsTuple],
...
def get_optimal_threshold(y_true,
    ...
    scores = np.array([nan_f1_score(y_true, y_prob >= t) for t in thresholds])
...
def get_opt_binary_attributions(atts_true: List[GraphsTuple],
                                atts_pred: List[GraphsTuple],
                                metric=nodewise_f1_score,
                                n_steps=20) -> List[GraphsTuple]:
