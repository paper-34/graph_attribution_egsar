...
class GNN(snt.Module, templates.TransparentModel):
    ...
    @tf.function(experimental_relax_shapes=True)
    def get_node_embeddings(self, x:GraphsTuple) -> tf.Tensor:
        return self.gnn(self.encode(x)).nodes
...
def get_batched_attributions(method: AttributionTechnique,
    ...
    for chunk in more_itertools.chunked(range(n), actual_batch_size):
        ...
        if 'EGSAR' in method.name:
            att = method.attribute(x_chunk, model, train_data, task_name, reg, 8)
        else:
...
def generate_result(model: templates.TransparentModel,
    ...
    if pred_atts is None:
        pred_atts = get_batched_attributions(method, model, inputs, train_data, task.name, reg, batch_size) # task.name is used for EGSAR
